...
Single source of truth for all data structures shared across packages and the Electron app. Both
`@self-review/core` and `@self-review/react` depend on this package. The Electron app's
...
- **Types only.** No runtime code, no utility functions, no constants. If it emits JavaScript, it
