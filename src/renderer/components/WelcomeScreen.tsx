...
export default function WelcomeScreen() {
  ...
  const handleOpenRemoteUrl = async () => {
    ...
    try {
      ...
      setRemoteError(err instanceof Error ? err.message : 'Failed to open remote PR/MR');
      setLoading(false);
    }
  };
      ...
      <div className='w-full max-w-lg space-y-6 p-6'>
        <div className='text-center space-y-4'>
          ...
          <div className='space-y-2'>
            ...
            <p className='text-muted-foreground'>
              GitHub-style PR review UI for local git diffs. Designed for solo developers reviewing
              AI-generated code.
            </p>
          </div>
        </div>
        ...
        <Card>
          ...
          <CardContent className='space-y-4'>
            ...
            {remoteError && (
              <p data-testid='remote-url-error' className='text-sm text-destructive'>
                {remoteError}
              </p>
            )}
          </CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Shows all files as new additions — useful for reviewing generated code
            </CardDescription>
          </CardHeader>
          <CardContent className='space-y-4'>
            <div className='flex items-center gap-3'>
            ...
            {selectedPath && (
              <Button onClick={handleStartReview} disabled={loading} className='w-full'>
                {loading ? 'Loading...' : 'Start Review'}
              </Button>
            )}
            {error && (
              <p data-testid='welcome-error' className='text-sm text-destructive'>
                {error}
              </p>
            )}
          </CardContent>
