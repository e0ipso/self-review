...
Then('the diff viewer should scroll to the {string} file section', async ({}, filePath: string) => {
  const page = getPage();
...
Then('the {string} entry in the file tree should be highlighted', async ({}, filePath: string) => {
  const page = getPage();
  const entry = page.locator(`[data-testid="file-entry-${filePath}"]`);
  await expect(entry).toHaveClass(/bg-accent/);
});

When('I scroll the diff viewer to the {string} file section', async ({}, filePath: string) => {
  const page = getPage();
  const section = page.locator(`[data-testid="file-section-${filePath}"]`);
...
When('I collapse the file tree', async () => {
  const page = getPage();
  await page.locator('[data-testid="file-tree-toggle"]').click();
  await expect(page.locator('[data-testid="file-tree-toggle"]')).toHaveAttribute(
    'aria-expanded',
    'false'
  );
});

When('I press {string}', async ({}, key: string) => {
  await getPage().keyboard.press(key);
});

Then('the file tree should be expanded', async () => {
  const page = getPage();
  await expect(page.locator('[data-testid="file-tree-toggle"]')).toHaveAttribute(
    'aria-expanded',
    'true'
...
Then('hint labels should be shown over the file tree', async () => {
  const page = getPage();
  const labels = page.locator('[data-testid="hint-overlay"] [data-testid^="hint-label-"]');
  await expect(labels.first()).toBeVisible();

  // Labels sit at their target's rect, so one inside the panel's box means
  // the collector measured the restored tree rather than the collapsed one.
  const panelBox = await page.locator('#fileTree').boundingBox();
  const labelBox = await labels.first().boundingBox();
  expect(panelBox).not.toBeNull();
  expect(labelBox).not.toBeNull();
  expect(labelBox!.x).toBeGreaterThanOrEqual(panelBox!.x);
  expect(labelBox!.x).toBeLessThan(panelBox!.x + panelBox!.width);
});
