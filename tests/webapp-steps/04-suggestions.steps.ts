...
When('I type {string} in the proposed code editor', async ({}, code: string) => {
  const page = getPage();
...
Then(
  ...
  async ({}, line: number, filePath: string) => {
    ...
    await expect(page.locator('[data-testid="suggestion-original"] textarea')).toHaveValue(
      expected
    );
    await expect(page.locator('[data-testid="suggestion-proposed"] textarea')).toHaveValue(
      expected
    );
  }
);

Then('the proposed code editor should contain {string}', async ({}, expected: string) => {
  const page = getPage();
  const value = await page.locator('[data-testid="suggestion-proposed"] textarea').inputValue();
