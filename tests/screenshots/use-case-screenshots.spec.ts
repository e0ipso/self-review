...
async function submitComment(commentInput: ReturnType<Page['locator']>): Promise<void> {
  await commentInput.locator('[data-testid="add-comment-btn"]').click();
}
...
test.describe('Use Case Screenshots', () => {
  test('UC1: Plan Review', async () => {
    ...
    try {
      ...
      const planSection = page.locator('[data-testid="file-section-plan.md"]');
      ...
      const commentInput = planSection.locator('[data-testid="comment-input"]').first();
  ...
  test('UC2: Code Review', async () => {
    ...
    try {
      ...
      const commentInput = loginSection.locator('[data-testid="comment-input"]').first();
      await typeInCommentEditor(
        page,
        commentInput,
      ...
      const suggestInput = loginSection.locator('[data-testid="comment-input"]').first();
      await typeInCommentEditor(
        page,
        suggestInput,
      ...
      const suggestBtn = suggestInput.locator('[data-testid="add-suggestion-btn"]');
      await suggestBtn.waitFor({ state: 'visible', timeout: 5000 });
      await suggestBtn.click();
      await page.waitForTimeout(300);

      // Fill in the proposed code
      const proposedEditor = suggestInput.locator('[data-testid="suggestion-proposed"] textarea');
      await proposedEditor.waitFor({ state: 'visible', timeout: 3000 });
      await proposedEditor.fill('    return { success: false, reason: "invalid_password" };');
      await page.waitForTimeout(200);
  ...
  test('UC3: Codebase Exploration', async () => {
    ...
    try {
      ...
      const section1 = page.locator('[data-testid="file-section-src/auth/middleware.ts"]');
      const input1 = section1.locator('[data-testid="comment-input"]').first();
      const catSelector1 = input1.locator('[data-testid="category-selector"]');
      await catSelector1.click();
      await page.waitForTimeout(200);
      await page.locator('[data-testid="category-option-question"]').first().click();
      await page.waitForTimeout(200);
      await typeInCommentEditor(
        page,
        input1,
      ...
      const helpersEntry = page.locator('[data-testid="file-entry-src/utils/helpers.ts"]');
      await helpersEntry.click();
      await page.waitForTimeout(500);
      await triggerCommentIcon(page, 'src/utils/helpers.ts', 7, 'new');
      const section2 = page.locator('[data-testid="file-section-src/utils/helpers.ts"]');
      const input2 = section2.locator('[data-testid="comment-input"]').first();
      const catSelector2 = input2.locator('[data-testid="category-selector"]');
      await catSelector2.click();
      await page.waitForTimeout(200);
      await page.locator('[data-testid="category-option-improvement"]').first().click();
      await page.waitForTimeout(200);
      await typeInCommentEditor(page, input2, 'Consider making the expiry time configurable');
      await submitComment(input2);
      await waitForInputsClosed(page);

      // Comment 3: needs-docs on docs/api-guide.md
      const docsEntry = page.locator('[data-testid="file-entry-docs/api-guide.md"]');
      await docsEntry.click();
      await page.waitForTimeout(500);
      await triggerCommentIcon(page, 'docs/api-guide.md', 13, 'new');
      const section3 = page.locator('[data-testid="file-section-docs/api-guide.md"]');
      const input3 = section3.locator('[data-testid="comment-input"]').first();
      const catSelector3 = input3.locator('[data-testid="category-selector"]');
      await catSelector3.click();
      await page.waitForTimeout(200);
      await page.locator('[data-testid="category-option-needs-docs"]').first().click();
      await page.waitForTimeout(200);
      await typeInCommentEditor(page, input3, 'Add rate limit headers to the API documentation');
      ...
      const fileCommentBtn = page.locator('[data-testid="add-file-comment-docs/api-guide.md"]');
      await fileCommentBtn.scrollIntoViewIfNeeded();
      await fileCommentBtn.click();
      await page.waitForTimeout(300);
      const input4 = section3.locator('[data-testid="comment-input"]').first();
      const catSelector4 = input4.locator('[data-testid="category-selector"]');
  ...
  test('UC4: AI-Assisted Review', async () => {
    ...
    try {
      ...
      ({ electronApp } = await launchElectron(['--resume-from', xmlPath], repoDir));
      const page = await electronApp.firstWindow();
