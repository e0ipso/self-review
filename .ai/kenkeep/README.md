...
3. **Review.** The changes show up in `git status` like any other code change. Inspect with `git diff`, accept with `git commit`, reject with `git restore <file>`. After node edits, run `npx kenkeep index rebuild` to refresh navigation. The repository lint-staged configuration formats Markdown but does not rebuild kenkeep indices.
...
- `type`: `practice` (how we build things: conventions, prohibitions, gotchas) or `map` (what exists in the project: features, vocabulary, locations).
- `tags`: free-form labels grouped under `## By topic` in `ENTRY.md`.
- `kk_derived_from`: list of session log filenames or doc paths that produced or refined this node. (Note: `_sessions/` is gitignored by default, so provenance only resolves for the original contributor unless your team commits it.)
- `kk_relates_to`: cross-references rendered in `GRAPH.md`.
- `description`: ≤140-character one-liner injected via `ENTRY.md`. Git history is the timeline of record for when a node was written or rewritten.

## Manually adding a node

Two paths, both human-in-the-loop via git:

- From the terminal: `npx kenkeep node add` (launches the kk-add skill).
- From inside a Claude Code session: `/kk-add`.

Either way the result lands in `nodes/<topic>/<id>.md`. Review with `git diff` and commit to accept.
...
- `nodes/`: knowledge nodes, organized in nested topical folders with generated `index.md` navigation. Reviewed via git.
...
- `ENTRY.md`: token-budgeted summary; injected into every new session. Regenerated by `npx kenkeep index rebuild`.
- `GRAPH.md`: full edge listing of nodes; available for the harness to read on demand. Regenerated by `npx kenkeep index rebuild`.
